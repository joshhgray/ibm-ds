...
def get_scatter_plot(site, payload):
    if site == 'All Sites':
        fig = px.scatter(
            spacex_df[spacex_df['Payload Mass (kg)'].between(payload[0], payload[1])],
            x='Payload Mass (kg)',
            y='class',
        ...
        fig = px.scatter(
            ...
            y='class',
            color='Booster Version Category',
            title='Payload vs Success at {}'.format(site)
        )
    return fig
